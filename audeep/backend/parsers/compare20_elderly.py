...
"""Parser for the ComParE 2020 USOMS-e (E) dataset"""
import abc
...
class Compare20ElderlyParser(LoggingMixin, Parser):
    ...
    def label_map(self) -> Optional[Mapping[str, int]]:
        if not self.can_parse():
            raise IOError("inable to parse the ComParE 2020 USOMS-e dataset at {}".format(self._basedir))

        return _COMPARE20_ELDERLY_LABEL_MAP
